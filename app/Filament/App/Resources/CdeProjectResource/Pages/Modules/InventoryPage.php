...
class InventoryPage extends BaseModulePage implements HasTable
{
    ...
    public function getStats(): array
    {
        ...
        $pendingPOs = PurchaseOrder::where('company_id', $companyId)->whereIn('status', ['submitted', 'approved', 'ordered'])->count();
        ...
        return [
            ...
            [
                ...
                'value' => PurchaseOrder::where('company_id', $companyId)->whereIn('status', ['ordered', 'partially_received'])->count(),
                'sub' => 'In transit',
                'sub_type' => 'info',
    ...
    protected function getPOFormSchema(): array
    {
        $companyId = $this->record->company_id;
        return [
            Section::make('Order Details')->schema([
                Forms\Components\TextInput::make('po_number')->label('PO Number')->required()
                ...
                Forms\Components\Select::make('supplier_id')->label('Supplier')
                    ->options(Supplier::where('company_id', $companyId)->pluck('name', 'id'))->searchable()->required(),
            ...
            Section::make('Amounts')->schema([
                ...
                Forms\Components\TextInput::make('total_amount')->numeric()->prefix('$')->default(0)->required()->label('Total'),
            ])->columns(4),
        ...
        return $table
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('status')->badge()
                    ->color(fn(string $state) => match ($state) { 'received' => 'success', 'ordered' => 'info', 'approved' => 'primary', 'submitted' => 'warning', 'partially_received' => 'warning', 'cancelled' => 'danger', default => 'gray'}),
                Tables\Columns\TextColumn::make('total_amount')->formatStateUsing(CurrencyHelper::formatter())->sortable()->label('Total'),
            ...
            ->filters([Tables\Filters\SelectFilter::make('status')->options(PurchaseOrder::$statuses)])
            ->headerActions([
                Action::make('create')->label('New Purchase Order')->icon('heroicon-o-plus')
                    ->schema($this->getPOFormSchema())
