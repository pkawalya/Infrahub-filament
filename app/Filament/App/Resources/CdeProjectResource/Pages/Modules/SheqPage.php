    ...
    protected function getIncidentFormSchema(): array
    {
        ...
        return [
            Section::make('Incident Details')->schema([
                Forms\Components\TextInput::make('incident_number')->label('Incident #')->required()
                    ->default(fn() => 'INC-' . str_pad((string) (SafetyIncident::where('cde_project_id', $this->record->id)->count() + 1), 4, '0', STR_PAD_LEFT)),
                Forms\Components\TextInput::make('title')->required()->maxLength(255),
                Forms\Components\Select::make('type')->options(['near_miss' => 'Near Miss', 'first_aid' => 'First Aid', 'injury' => 'Injury', 'property_damage' => 'Property Damage', 'environmental' => 'Environmental', 'fire' => 'Fire', 'other' => 'Other'])->required(),
                Forms\Components\Select::make('severity')->options(['minor' => 'Minor', 'moderate' => 'Moderate', 'major' => 'Major', 'critical' => 'Critical'])->required()->default('minor'),
                Forms\Components\Select::make('status')->options(SafetyIncident::$statuses)->required()->default('reported'),
                Forms\Components\DateTimePicker::make('incident_date')->required()->default(now()),
                Forms\Components\TextInput::make('location')->placeholder('e.g. Block A, Floor 3')->columnSpanFull(),
            ])->columns(2),
            Section::make('Details & Resolution')->schema([
                Forms\Components\Textarea::make('description')->rows(3)->columnSpanFull(),
                Forms\Components\Textarea::make('root_cause')->rows(2)->label('Root Cause'),
                Forms\Components\Textarea::make('corrective_action')->rows(2)->label('Corrective Action'),
                Forms\Components\Select::make('investigated_by')->label('Investigated By')
                    ->options(User::where('company_id', $companyId)->where('is_active', true)->pluck('name', 'id'))->searchable()->nullable(),
            ])->columns(2),
        ];
    }
    ...
    public function table(Table $table): Table
    {
        ...
        return $table
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('severity')->badge()->color(fn(string $state) => match ($state) { 'critical' => 'danger', 'major' => 'warning', 'minor' => 'info', default => 'gray'}),
                Tables\Columns\TextColumn::make('status')->badge()->color(fn(string $state) => match ($state) { 'resolved' => 'success', 'closed' => 'gray', 'investigating' => 'warning', 'reported' => 'info', default => 'gray'}),
