...
class TaskWorkflowPage extends BaseModulePage implements HasTable, HasForms
{
    ...
    {
        $pid = $this->pid();
        $base = Task::where('cde_project_id', $pid);

        $total = (clone $base)->count();
        $todo = (clone $base)->where('status', 'to_do')->count();
        ...
        $overdue = (clone $base)->whereNotIn('status', ['done', 'cancelled'])
            ->whereNotNull('due_date')->where('due_date', '<', now())->count();
        $avgProg = $total > 0 ? round((clone $base)->avg('progress_percent') ?? 0) : 0;
        ...
        return [
            ...
            [
                'label' => 'Avg. Progress',
                'value' => $avgProg . '%',
                'sub' => $todo . ' to do · ' . $inProgress . ' active',
        ...
        return [
            Section::make('Task Details')->schema([
                ...
                Forms\Components\Select::make('parent_id')->label('Parent Task')
                    ->options(fn() => Task::where('cde_project_id', $this->pid())->pluck('title', 'id'))
                    ->searchable()->nullable(),
                Forms\Components\DatePicker::make('start_date'),
                Forms\Components\DatePicker::make('due_date'),
                Forms\Components\TextInput::make('estimated_hours')->numeric()->suffix('hrs'),
                Forms\Components\TextInput::make('actual_hours')->numeric()->suffix('hrs')->visible(!$isCreate),
                Forms\Components\TextInput::make('progress_percent')->label('Progress (%)')->numeric()
                    ->minValue(0)->maxValue(100)->suffix('%')->default(0),
            ])->columns(2),
    ...
    protected function getHeaderActions(): array
    {
        return [
            Action::make('createTask')
                ->label('New Task')->icon('heroicon-o-plus-circle')->color('primary')
                ->modalWidth('3xl')
                ->schema($this->taskFormSchema(isCreate: true))
    ...
    public function table(Table $table): Table
    {
        return $table
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('estimated_hours')->label('Est.')->suffix('h')->placeholder('—')->toggleable(),
                ...
                Tables\Columns\TextColumn::make('due_date')->date()->sortable()
                    ...
                    ->description(fn($record) => $record->due_date?->isPast() && !in_array($record->status, ['done']) ? 'Overdue' : null),
            ...
            ->filters([
                ...
                Tables\Filters\Filter::make('has_parent')->label('Sub-tasks Only')
                    ->query(fn($q) => $q->whereNotNull('parent_id'))->toggle(),
            ])
            ->recordActions([
