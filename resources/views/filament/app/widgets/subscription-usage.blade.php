...
<div
    x-data="{
        open: JSON.parse(localStorage.getItem('widget_subscription_open') ?? 'true'),
        toggle() { this.open = !this.open; localStorage.setItem('widget_subscription_open', this.open); }
    }"
...
>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
        ...
        {{-- Card Header — doubles as collapsible toggle --}}
        <button
            type="button"
            @click="toggle()"
            class="w-full flex items-center justify-between px-5 py-3 border-b border-gray-100 dark:border-gray-800 group hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors"
            :aria-expanded="open"
            aria-controls="subscription-widget-body"
        >
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="flex items-center gap-1.5 text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-300 transition-colors">
                    <span class="text-[10px] font-semibold uppercase tracking-wider" x-text="open ? 'Hide' : 'Show'"></span>
        ...
        {{-- Collapsible meters grid --}}
        <div
            id="subscription-widget-body"
            x-show="open"
            x-collapse
            x-cloak
        >
