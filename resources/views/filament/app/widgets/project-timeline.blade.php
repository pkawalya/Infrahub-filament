            ...
            <div class="timeline-container overflow-x-auto">
                ...
                <div class="flex border-b border-gray-200 dark:border-gray-700 mb-1 min-w-[700px]">
                    @foreach($months as $month)
                        <div class="text-xs font-medium px-1 py-1.5 text-center border-r border-gray-100 dark:border-gray-800 truncate
                ...
                <div class="relative min-w-[700px]" style="min-height: {{ count($projects) * 68 + 20 }}px;">
                    ...
                    <div class="absolute inset-0 flex pointer-events-none">
                        @foreach($months as $month)
                            <div class="border-r border-gray-100 dark:border-gray-800 h-full"
                    ...
                    @if($todayPercent !== null)
                        <div class="absolute top-0 bottom-0 z-10 pointer-events-none" style="left: {{ $todayPercent }}%;">
                            ...
                            <div
                                class="absolute -top-0.5 -left-2.5 bg-red-500 text-white text-[9px] font-bold px-1 rounded shadow">
                                TODAY
                            </div>
                        </div>
                    @endif
                    ...
                    @foreach($projects as $index => $project)
                        <div class="absolute left-0 right-0" style="top: {{ $index * 68 + 10 }}px; height: 58px;">
                            {{-- Project Label --}}
                            <a href="{{ $project['url'] }}" class="group absolute flex items-center rounded-lg shadow-sm border transition-all duration-200
                                ...
                                @if($project['expectedProgress'] > 0 && $project['expectedProgress'] < 100)
                                    <div class="absolute inset-y-0 z-[5] pointer-events-none"
                                        ...
                                        <div
                                            class="w-[2px] h-full border-l-2 border-dashed border-gray-400 dark:border-gray-500 opacity-60">
                                        </div>
                                        <div class="absolute -top-0.5 -left-[5px]">
                                            <div
                                                class="w-0 h-0 border-l-[5px] border-r-[5px] border-t-[5px] border-l-transparent border-r-transparent border-t-gray-400 dark:border-t-gray-500 opacity-80">
                                            </div>
                                        </div>
                                    </div>
                                @endif
                                ...
                                <div class="absolute inset-y-0 left-0 rounded-l-lg transition-all duration-500" style="width: {{ $project['progress'] }}%;
                                                   background-color: {{ $project['color'] }};
                                                   opacity: 0.22;"></div>

                                {{-- Content --}}
                                <div class="relative flex items-center justify-between w-full px-2.5 py-1 z-10">
                                    <div class="flex items-center gap-2 min-w-0">
                                        ...
                                        <div class="min-w-0">
                                            <div
                                                class="text-xs font-semibold text-gray-900 dark:text-gray-100 truncate group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">
                                                {{ $project['name'] }}
                                            </div>
                                    ...
                                    <div class="flex items-center gap-2 ml-2 flex-shrink-0">
                                        <div class="text-right leading-tight">
                                            <div class="text-[11px] font-bold" style="color: {{ $project['color'] }};">
                                                {{ $project['progress'] }}%
                                            </div>
                                ...
                                @foreach($project['milestones'] as $milestone)
                                    ...
                                    @endphp
                                    <div class="absolute z-10 group/ms" style="left: {{ $relPos }}%; bottom: -2px;"
                                        ...
                                        {{-- Tooltip --}}
                                        <div
                                            class="hidden group-hover/ms:block absolute bottom-5 left-1/2 -translate-x-1/2 bg-gray-900 dark:bg-gray-700 text-white text-[10px] px-2 py-1 rounded shadow-lg whitespace-nowrap z-50 pointer-events-none">
                ...
                {{-- Legend --}}
                <div
                   class="flex flex-wrap items-center gap-x-4 gap-y-1 mt-4 pt-3 border-t border-gray-100 dark:border-gray-800 text-[10px] text-gray-500 dark:text-gray-400">
                    <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-emerald-500 inline-block"></span> On Track / Ahead</span>
                    <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-red-500 inline-block"></span> Behind Schedule</span>
                    ...
                    <span class="flex items-center gap-1 border-l border-gray-200 dark:border-gray-700 pl-3">
                        <span class="w-3 border-t-2 border-dashed border-gray-400 inline-block"></span> Expected
                    </span>
                    <span class="flex items-center gap-1">
                        <span class="w-3 h-0.5 bg-red-500 inline-block opacity-60"></span> Today
                    </span>
