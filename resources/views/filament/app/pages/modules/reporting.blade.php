        ...
        <div style="display:grid; grid-template-columns: repeat(4, 1fr); gap:1rem; margin-bottom:1rem;">
            <div>
                ...
                <div style="font-size:1.25rem; font-weight:700;" class="dark:text-gray-100">
                    {{ \App\Support\CurrencyHelper::format($fin['contract_original'], 0) }}
                </div>
            </div>
            <div>
                <div
                ...
                <div
                    ...
                    {{ \App\Support\CurrencyHelper::format($fin['contract_revised'], 0) }}
                    @if($fin['variance'] != 0)
                        <span
                            style="font-size:11px;">{{ $fin['variance'] > 0 ? '+' : '' }}{{ \App\Support\CurrencyHelper::format($fin['variance'], 0) }})</span>
                    @endif
                </div>
            </div>
            <div>
                <div
                    ...
                    BOQ Value</div>
                <div style="font-size:1.25rem; font-weight:700;" class="dark:text-gray-100">
                    {{ \App\Support\CurrencyHelper::format($fin['boq_value'], 0) }}
                </div>
            </div>
            <div>
                <div
                    ...
                    PO Spend</div>
                <div style="font-size:1.25rem; font-weight:700;" class="dark:text-gray-100">
                    {{ \App\Support\CurrencyHelper::format($fin['po_total'], 0) }}
                </div>
            </div>
        </div>
        {{-- Payment progress bar --}}
        <div style="font-size:12px; font-weight:600; color:#6b7280; margin-bottom:4px;" class="dark:text-gray-400">
            Payment Progress — {{ $fin['contract_pct'] }}% paid
            ({{ \App\Support\CurrencyHelper::format($fin['contract_paid'], 0) }}
            of
            {{ \App\Support\CurrencyHelper::format($fin['contract_revised'], 0) }})
        </div>
