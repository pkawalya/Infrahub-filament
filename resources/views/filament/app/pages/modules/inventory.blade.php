<x-filament-panels::page>
    ...
    {{-- PO Pipeline Status Bar --}}
    @php $poPipe = $this->getPOPipeline(); @endphp
    <div style="display:flex;gap:2px;background:white;border:1px solid #e2e8f0;border-radius:8px;padding:4px;margin-bottom:10px;overflow-x:auto;">
        @foreach($poPipe as $p)
            <div style="flex:1;min-width:70px;text-align:center;padding:6px 4px;border-radius:6px;background:{{ $p['bg'] }};color:{{ $p['color'] }};">
    ...
    {{-- Low Stock Alert --}}
    @php $reorderItems = $this->getReorderAlerts(); @endphp
    @if($reorderItems->isNotEmpty())
        <div style="display:flex;align-items:center;gap:8px;padding:8px 14px;border-radius:6px;background:#fef2f2;border:1px solid #fecaca;color:#991b1b;font-size:12px;margin-bottom:10px;">
            <svg style="width:16px;height:16px;flex-shrink:0;" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z"/></svg>
            <strong>{{ $reorderItems->count() }} item{{ $reorderItems->count() > 1 ? 's' : '' }} below reorder level:</strong>
            {{ $reorderItems->take(5)->pluck('name')->join(', ') }}{{ $reorderItems->count() > 5 ? '…' : '' }}
        </div>
    @endif
    ...
    @if($activeInventoryTab === 'issuances')
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
            <div style="font-size:11px; color:#9ca3af; font-style:italic;">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" style="width:14px;height:14px;display:inline;vertical-align:text-bottom;">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" />
                </svg>
                Materials can only be issued against approved requisitions
            </div>
            ...
            <div style="background:white;border-radius:16px;padding:24px;width:100%;max-width:550px;max-height:90vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,.2)"
                ...
                <form wire:submit="submitIssuance">
                    <div style="display:grid;gap:12px">
                        ...
                        <div style="background:#fffbeb;border:1px solid #fbbf24;border-radius:10px;padding:12px">
                            <label style="display:block;font-size:12px;font-weight:700;margin-bottom:6px;color:#92400e">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" style="width:14px;height:14px;display:inline;vertical-align:text-bottom;">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
                                </svg>
                                Approved Requisition * <span style="font-weight:400;color:#b45309">(required)</span>
                            </label>
                            @php $reqOptions = $this->getApprovedRequisitionOptions(); @endphp
                            @if(count($reqOptions) > 0)
                                <x-searchable-select :wire-model="'issuanceForm.requisition_id'"
                                    :options="$reqOptions" placeholder="-- Select Approved Requisition --" :required="true" />
                            @else
                                <div style="padding:10px;background:#fef2f2;border-radius:8px;font-size:12px;color:#991b1b;text-align:center;">
                                    No approved requisitions available. Create and approve a requisition first in the <strong>Requisitions</strong> tab.
                                </div>
                            @endif
                        </div>
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
